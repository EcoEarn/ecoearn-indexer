...
public class TokensPoolRewardPerSecondSetLogEventProcessor : AElfLogEventProcessorBase<TokensPoolRewardPerSecondSet,
    LogEventInfo>
{
    private readonly IObjectMapper _objectMapper;
    private readonly ContractInfoOptions _contractInfoOptions;
    private readonly ILogger<TokensPoolRewardPerSecondSetLogEventProcessor> _logger;
    ...
    public TokensPoolRewardPerSecondSetLogEventProcessor(
        ILogger<TokensPoolRewardPerSecondSetLogEventProcessor> logger,
    ...
    protected override async Task HandleEventAsync(TokensPoolRewardPerSecondSet eventValue,
        LogEventContext context)
    {
        try
        {
            _logger.Debug("TokensPoolRewardPerSecondSet: {eventValue} context: {context}",
            ...
            tokenPoolIndex.TokenPoolConfig.RewardPerBlock = eventValue.RewardPerSecond;
            _objectMapper.Map(context, tokenPoolIndex);
            await _tokenPoolRepository.AddOrUpdateAsync(tokenPoolIndex);
        }
        catch (Exception e)
        {
            _logger.LogError(e, "TokensPoolRewardPerSecondSet HandleEventAsync error.");
        }
    }
}