...
public class TokensPoolFixedBoostFactorSetLogEventProcessor : AElfLogEventProcessorBase<TokensPoolFixedBoostFactorSet,
    LogEventInfo>
{
    private readonly IObjectMapper _objectMapper;
    private readonly ContractInfoOptions _contractInfoOptions;
    private readonly ILogger<TokensPoolFixedBoostFactorSetLogEventProcessor> _logger;
    ...
    public TokensPoolFixedBoostFactorSetLogEventProcessor(
        ILogger<TokensPoolFixedBoostFactorSetLogEventProcessor> logger,
    ...
    protected override async Task HandleEventAsync(TokensPoolFixedBoostFactorSet eventValue,
        LogEventContext context)
    {
        try
        {
            _logger.Debug("TokensPoolFixedBoostFactorSet: {eventValue} context: {context}",
            ...
            tokenPoolIndex.TokenPoolConfig.FixedBoostFactor = eventValue.FixedBoostFactor;
            _objectMapper.Map(context, tokenPoolIndex);
            await _tokenPoolRepository.AddOrUpdateAsync(tokenPoolIndex);
        }
        catch (Exception e)
        {
            _logger.LogError(e, "TokensPoolFixedBoostFactorSet HandleEventAsync error.");
        }
    }
}